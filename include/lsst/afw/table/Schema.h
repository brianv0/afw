...
public:
     ...
     */
    template <typename F>
    void find(std::string const & name, F && func) const {
        _impl->find(_aliases->apply(name), std::forward<F>(func));
    }
...
public:
    ...
    template <typename F>
    void find(std::string const & name, F && func) const {
        _impl->find(_aliases->apply(join(_name, name)), std::forward<F>(func));
    }
    ...
    void apply(F && func) const {
        _impl->find(_aliases->apply(_name), std::forward<F>(func));
    }
